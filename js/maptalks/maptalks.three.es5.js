  ...
  import * as maptalks from '/js/maptalks/maptalks.es.js'
  ...
  /*#__PURE__*/
  function (_THREE$InstancedBuffe) {
    ...
    function LineSegmentsGeometry() {
      ...
      addAttribute(_assertThisInitialized(_assertThisInitialized(_this)), 'position', new THREE__namespace.Float32BufferAttribute(positions, 3));
      addAttribute(_assertThisInitialized(_assertThisInitialized(_this)), 'uv', new THREE__namespace.Float32BufferAttribute(uvs, 2));
      return _this;
    ...
    _proto.setPositions = function setPositions(array) {
      ...
      addAttribute(this, 'instanceEnd', new THREE__namespace.InterleavedBufferAttribute(instanceBuffer, 3, 3)); // this.addAttribute('instanceStart', new THREE.InterleavedBufferAttribute(instanceBuffer, 3, 0)); // xyz
    ...
    _proto.setColors = function setColors(array) {
      ...
      addAttribute(this, 'instanceColorEnd', new THREE__namespace.InterleavedBufferAttribute(instanceColorBuffer, 3, 3)); // this.addAttribute('instanceColorStart', new THREE.InterleavedBufferAttribute(instanceColorBuffer, 3, 0)); // rgb
  ...
  ShaderLib['line'] = {
    uniforms: THREE__namespace.UniformsUtils.merge([THREE__namespace.UniformsLib.common, THREE__namespace.UniformsLib.fog, UniformsLib.line]),
  ...
  function (_THREE$Mesh) {
    ...
    _proto.computeLineDistances = function computeLineDistances() {
      ...
      addAttribute(geometry, 'instanceDistanceEnd', new THREE__namespace.InterleavedBufferAttribute(instanceDistanceBuffer, 1, 1)); // geometry.addAttribute('instanceDistanceStart', new THREE.InterleavedBufferAttribute(instanceDistanceBuffer, 1, 0)); // d0
  ...
  function getDefaultCylinderBufferGeometry(radialSegments) {
    ...
    if (!barGeometryCache[radialSegments]) {
      var geometry = new THREE__namespace.CylinderBufferGeometry(1, 1, 1, radialSegments, 1);
  ...
  function mergeBarGeometry(geometries) {
    ...
    for (var _i = 0, _len = geometries.length; _i < _len; _i++) {
      var _geometries$_i$attrib = geometries[_i].attributes,
          color = _geometries$_i$attrib.color,
      ...
      if (color) {
        ...
        offset += color.array.length; // for (let j = 0, len1 = color.array.length; j < len1; j++) {
    ...
    if (colors.length) {
      addAttribute(bufferGeometry, 'color', new THREE__namespace.BufferAttribute(colors, 3, true)); // for (let i = 0, len = colors.length; i < len; i++) {
  ...
  */
  ...
  if (maptalks__namespace.worker) {
    ...
    function (_maptalks$worker$Acto) {
      ...
      _proto.pushQueue = function pushQueue(q) {
        ...
        } else if (type === 'Point') ; else if (type === 'Line' || type === 'FatLine') {
  ...
  function gengerateExtrudePolygons(polygons, center, layer, options) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var p = polygon;
      var properties = options[i] ? options[i] : isGeoJSONPolygon(p) ? p['properties'] : p.getProperties() || {};
  ...
  function gengerateExtrudeLines(lineStringList, center, layer, lineStrings, options) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var properties = options[i] ? options[i] : isGeoJSONLine(lineStrings[i]) ? lineStrings[i]['properties'] : lineStrings[i].getProperties() || {};
    ...
    for (var i = 0; i < len; i++) {
      ...
      for (var j = 0, len1 = multiLineString.length; j < len1; j++) {
        ...
        if (isMercator) {
          ...
          var _arrayBuffer = getLinePosition(lineString, layer, center, false).arrayBuffer;
  ...
  var ExtrudePolygonTask =
  /*#__PURE__*/
  function (_BaseObjectTask) {
  ...
  var ExtrudePolygonsTask =
  /*#__PURE__*/
  function (_BaseObjectTask2) {
  ...
  var ExtrudeLinesTask =
  /*#__PURE__*/
  function (_BaseObjectTask4) {
    ...
    function Line(lineString, options, material, layer) {
      ...
      if (asynchronous) {
        ...
        LineTaskIns.push({
          ...
          key: options.key,
          lineString: lineString,
          baseObject: _assertThisInitialized(_assertThisInitialized(_this))
        });
      } else {
        var positionList = [];
        ...
        for (var i = 0, len = lineStrings.length; i < len; i++) {
          ...
          var _getLinePosition = getLinePosition(_lineString, layer, center, false),
  ...
  var EVENTS$1 = ['click', 'mousemove', 'mousedown', 'mouseup', 'dblclick', 'contextmenu'].join(' ').toString();
    ...
    return (
      ...
      function (_Base) {
        ...
        _proto._setPickObject3d = function _setPickObject3d() {
          ...
          addAttribute(geometry, 'color', new THREE__namespace.BufferAttribute(colors, 3, true)); // const material = new THREE.MeshBasicMaterial();
  ...
  function (_MergedMixin) {
    ...
    function ExtrudePolygons(polygons, options, material, layer) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        var _center = polygon.getCenter ? polygon.getCenter() : getGeoJSONCenter(polygon, TEMP_COORD$1);
      ...
      var center = new maptalks__namespace.Coordinate((minX + maxX) / 2, (minY + maxY) / 2);
    ...
    function ExtrudeLines(lineStrings, options, material, layer) {
      ...
      if (asynchronous) {
        ...
        for (var _i = 0; _i < len; _i++) {
          ...
          var opts = maptalks__namespace.Util.extend({}, OPTIONS$9, isGeoJSON(_lineString) ? _lineString['properties'] : _lineString.getProperties(), {
            index: _i
          });
  ...
  function (_MergedMixin) {
      ...
      if (asynchronous) {
        ...
        for (var _i = 0; _i < len; _i++) {
          ...
          for (var m = 0, le = lls.length; m < le; m++) {
            ...
            var _getLinePosition = getLinePosition(lls[m], layer, center, false),
                positions = _getLinePosition.positions;
    ...
    _proto.getSelectMesh = function getSelectMesh() {
      ...
      if (index != null) {
        if (!this._baseObjects[index]) {
          var lineString = this._datas[index];
          var opts = maptalks__namespace.Util.extend({}, this.getOptions(), {
            index: index
  ...
  */
  ...
  function (_maptalks$TileLayer) {
    ...
    function BaseVectorTileLayer(url, options) {
      ...
      _this = _maptalks$TileLayer.call(this, maptalks__namespace.Util.GUID(), maptalks__namespace.Util.extend({
        urlTemplate: url
      }, options)) || this;
      _this._opts = null;
      _this._layer = null;
  ...
  function (_BaseVectorTileLayer) {
    ...
    function ThreeVectorTileLayer(url, options, getMaterial, layer) {
      ...
      _this = _BaseVectorTileLayer.call(this, maptalks__namespace.Util.GUID(), maptalks__namespace.Util.extend({
        urlTemplate: url
      }, OPTIONS$7, options)) || this;
      _this._opts = options;
      _this._layer = layer;
      ...
      for (var layerName in json) {
        ...
        if (features && features.length) {
          ...
          for (var i = 0, len = features.length; i < len; i++) {
            ...
            if (isGeoJSONPolygon(feature)) {
              ...
              for (var _j = 0, _len = _fs.length; _j < _len; _j++) {
                points.push(maptalks__namespace.Util.extend({}, _fs[_j].properties, _fs[_j], {
          ...
          if (polygons.length) {
            ...
            if (material) {
              var extrudepolygons = this._layer.toExtrudePolygons(polygons, maptalks__namespace.Util.extend({}, {
                topColor: '#fff',
                layerName: layerName,
          ...
          if (lineStrings.length) {
            ...
            if (_material && (_material instanceof THREE__namespace.LineBasicMaterial || _material instanceof THREE__namespace.LineDashedMaterial)) {
              var lines = this._layer.toLines(lineStrings, maptalks__namespace.Util.extend({}, {
                layerName: layerName,
                asynchronous: asynchronous
              }, opts), _material);
          ...
          if (points.length) {
            ...
            if (_material2 && _material2 instanceof THREE__namespace.PointsMaterial) {
              var ps = this._layer.toPoints(points, maptalks__namespace.Util.extend({}, {
                layerName: layerName,
                asynchronous: asynchronous
              }, opts), _material2);
  ...
  function (_BaseObject) {
    ...
    function HeatMap(data, options, material, layer) {
      ...
      _this.getObject3d().position.copy(new THREE__namespace.Vector3((minX + maxX) / 2, (minY + maxY) / 2, z));
    ...
    function FatLine(lineString, options, material, layer) {
      ...
      if (asynchronous) {
        ...
        for (var m = 0, le = lineStrings.length; m < le; m++) {
          var positions = getLinePosition(lineStrings[m], layer, center, false).positions;
  ...
  var LINEPRECISIONS = [[4000, 220], [2000, 100], [1000, 30], [500, 15], [100, 5], [50, 2], [10, 1], [5, 0.7], [2, 0.1], [1, 0.05], [0.5, 0.02], [0.4, 0.01], [0.1, 0.005], [0.05, 0.002], [0.01, 0.001]];
  ...
  function (_maptalks$CanvasLayer) {
    ...
    _proto.drawOnInteracting = function drawOnInteracting(gl, view, scene, camera, event, timeStamp, context) {
      this.renderScene(context, this);
    };
    /**
    ...
    _proto.coordinateToVector3 = function coordinateToVector3(coordinate, z) {
      ...
      var p = coordinateToPoint(map, isArray ? TEMP_COORD : coordinate, res, TEMP_POINT);
      return new THREE__namespace.Vector3(p.x, p.y, z);
    };

    _proto.coordinatiesToGLFloatArray = function coordinatiesToGLFloatArray(coordinaties, centerPt) {
      var map = this.getMap();
    ...
    _proto.coordinatiesToGLArray = function coordinatiesToGLArray(coordinaties, centerPt) {
      var map = this.getMap();
    ...
    */
    ...
    _proto._identifyBaseObjectEvents = function _identifyBaseObjectEvents(e) {
      ...
      var count = Math.max(0, maptalks__namespace.Util.isNumber(identifyCountOnEvent) ? identifyCountOnEvent : 0);
  ...
  function (_maptalks$renderer$Ca) {
    ...
    _proto2._initThreeRenderer = function _initThreeRenderer() {
      ...
      var camera = this.camera = new THREE__namespace.PerspectiveCamera(fov, map.width / map.height, map.cameraNear, map.cameraFar);
    ...
    _proto2.resizeCanvas = function resizeCanvas(canvasSize) {
      ...
      var r = map.getDevicePixelRatio ? map.getDevicePixelRatio() : maptalks__namespace.Browser.retina ? 2 : 1;
