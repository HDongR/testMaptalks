define(['exports'], function (exports) { 'use strict';
    ...
    */
    ...
    var onmessage = function onmessage(message, postResponse) {
      ...
      if (type === 'Polygon') {
        generateData(datas);
        var result = generateExtrude(datas);
        ...
        var _result = generateExtrude(datas, true);

        postResponse(null, _result, [_result.position, _result.normal, _result.uv, _result.indices]);
      }
    };
    ...
    function generateExtrude(datas, isLine) {
      if (isLine === void 0) {
        isLine = false;
      }
      ...
      for (var i = 0; i < len; i++) {
        var buffGeom = isLine ? extrudeLine(datas[i]) : extrudePolygons(datas[i]);
        var minZ = datas[i].bottomHeight || 0;
    ...
    function extrudeLine(d) {
      ...
      var _extrudePolyline = extrudePolyline(data, {
        lineWidth: width,
        depth: height
      }),
